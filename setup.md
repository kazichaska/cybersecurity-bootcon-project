...
## 💻 Lab Setup on macOS Using Docker (No Docker Desktop)

### ✅ Step 1: Start Colima as the Docker Runtime Backend
```bash
colima start --cpu 2 --memory 4 --disk 20 --arch aarch64
```

Verify Docker is pointed to Colima:
```bash
...
- [ ] Lab setup slides: Docker (Colima-backed)
...
- Dockerized lab setup for repeatable demos using **Colima instead of Docker Desktop**
